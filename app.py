...
class PlayRedux(threading.Thread):
    ...
    def set_background(self):
        ...
        self.ye_bg = ye_mask_bg[15:55, 300:420]
        # self.ye_bg_t = ye_mask_bg[52:56, 280:340]
        
        self.bl_bg = bl_mask_bg[15:55, 450:570]
        # self.bl_bg_t = bl_mask_bg[52:56, 400:480]
        
        self.or_bg = or_mask_bg[15:55, 595:715]
        # self.or_bg_t = or_mask_bg[52:56, 540:620]
        
        self.pur_bg = pur_mask_bg[15:55, 10:60]
    ...
    def set_area(self):
        if (self.img_check is not None):
            ...
            # Green needs to be 2 higher due to star notes very rarely not hitting the box, thus missing the note.
            self.gn_chk = self.gn_mask[15:55, 10:130]
            # self.gn_chk_t = self.gn_mask[52:56, 10:70]
            # Red's second tracker needs to be higher because of star notes.
            self.r_chk = self.r_mask[15:55, 155:275]
            # self.r_chk_t = self.r_mask[49:56, 140:210]
            # Yellow needed to be 2 higher in order to have at least some number when checking if other notes were present.
            self.ye_chk = self.ye_mask[15:55, 300:420]
            # self.ye_chk_t = self.ye_mask[52:56, 280:340]
            
            self.bl_chk = self.bl_mask[15:55, 450:570]
            # self.bl_chk_t = self.bl_mask[52:56, 400:480]
            
            self.or_chk = self.or_mask[15:55, 595:715]
            # self.or_chk_t = self.or_mask[52:56, 540:620]
            
            self.pur_chk = self.pur_mask[15:55, 10:60]
    ...
    def check_colors(self):
        if(np.sum(self.pur_df) > 100 and current_time() - self.purple_strum > STRUM_TIME):
            ...
            self.played = True
            self.notes.append('p')
        if((np.sum(self.gn_df) > 200) and current_time() - self.green_strum > STRUM_TIME):
            self.green_time.append(current_time() - self.green_strum)
            self.green_strum = current_time()
            self.played = True
            self.notes.append('a')
        if((np.sum(self.r_df) > 200) and current_time() - self.red_strum > STRUM_TIME):
            self.red_time.append(current_time() - self.red_strum)
            self.red_strum = current_time()
            self.played = True
            self.notes.append('s')
        if((np.sum(self.ye_df) > 200)  and current_time() - self.yellow_strum > STRUM_TIME):
            self.yellow_time.append(current_time() - self.yellow_strum)
            self.yellow_strum = current_time()
            self.played = True
            self.notes.append('d')
        if((np.sum(self.bl_df) > 200) and current_time() - self.blue_strum > STRUM_TIME):
            self.blue_time.append(current_time() - self.blue_strum)
            self.blue_strum = current_time()
            self.played = True
            self.notes.append('f')   
        if((np.sum(self.or_df) > 200) and current_time() - self.orange_strum > STRUM_TIME):
    ...
    if key == start_stop_key:
        if play_thread.running:
            ...
            for x in play_thread.images:
                ...
                cv2.imwrite('data\images\img_{}_g{}_r{}_y{}_b{}_o{}'
                            '.png'.format(y, 
                                                                                        x['green'], x['red'], x['yellow'], x['blue'], x['orange'], x['played']), 
